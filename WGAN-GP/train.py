...
## variable setting
parser = argparse.ArgumentParser(description="WGAN-GP parameter",
...
## Optimizer
# optimizer = RMSprop(WGAN) => Adam(WGAN-GP)
...
if mode == 'train':
    ...
    for epoch in range(st_epoch+1, num_epoch+1):
        ...
        for batch, data in enumerate(loader_train, 1):
            ...
            # Gradient penalty
            gradient_penalty = compute_gradient_penalty(discriminator=netD, real_samples=label.data,fake_samples=output.data)

            # Adversarial loss
            loss_D = -torch.mean(netD(label)) + torch.mean(netD(output.deatch())) + lambda_gp * gradient_penalty
